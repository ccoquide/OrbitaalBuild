...
def writeNodeTable(sparksession, pydf, PATH, years=[y for y in range(2009,2022)]):
    ...
    getNewId(sparksession, fname, f"{PATH}/TEMPORAL/tmp.csv", f"{PATH}/TEMPORAL/tmp2.csv")
    
    #Turn csv temporary table with new IDs into parquet
    id_timestamp_df_src=sparksession.read.csv(f"{PATH}/TEMPORAL/tmp2.csv", header=True, inferSchema=True).orderBy("NEW_ID")
    print(id_timestamp_df_src.columns)
    id_timestamp_df_src.write.parquet(fname, mode="overwrite")
    
    #Remove temporary files
    os.remove(f"{PATH}/TEMPORAL/tmp2.csv")
